...
export class FactDsaApplicantDetailService {
  ...
  async findAllApplications(): Promise<ApiResponse<FactDsaApplicantDetail[]>> {
    let appval=null;
    console.log(appval);
    let fact_dsa_applicant_detail = await this.factDsaApplicantDetailRepository/* .query('select * from dsa.fact_dsa_applicant_details where applicant_id IS NOT NULL') */
    .createQueryBuilder()
